...
// 💡 修復：確保每次切換 selectedTier 都會重新計算 active_price
const displayProducts = computed(() => {
  let list = store.products.map(p => {
    ...
    // 動態抓取目前選擇的定價欄位，若沒有則 fallback 回原價
    const priceCol = tierMapping[selectedTier.value]
    const finalPrice = Number(p[priceCol]) || Number(p.retail_price) || 0
...
// 使用 active_price (已套用折扣的價格) 計算
const totalRevenue = computed(() => cart.value.reduce((sum, item) => sum + (item.active_price * item.qty), 0))
const totalCost = computed(() => cart.value.reduce((sum, item) => sum + ((Number(item.price_50) || Number(item.cost) || 0) * item.qty), 0))
...
async function finalizeCheckout(payeeName) {
  ...
  const { error: txnError } = await supabase.from('transactions').insert([{
    ...
    branch: selectedBranch.value.replace('店',''), client_id: selectedClient.value.id,
    handled_by: payeeName, staff: payeeName, note: `${selectedClient.value.name} (${itemsStr})`
  }])
  ...
  alert(`✅ 結帳成功！由 ${payeeName} 收取 $${totalRevenue.value}`)
...
<template>
  <div class="page" style="padding-bottom: 180px;">
    ...
    <div class="glass-card top-card">
      ...
      <div class="grid-2" style="margin-top:15px;">
        ...
        <div class="form-item"><label>3. 客戶折扣</label>
          <select v-model="selectedTier" class="modern-select highlight-sel"><option v-for="(col, key) in tierMapping" :key="key" :value="key">{{ key }}</option></select>
        </div>
      </div>
    </div>

    <div class="filter-bar">
      <div class="search-box"><span class="s-icon">🔍</span><input v-model="searchProduct" placeholder="即時搜尋商品..." class="s-inp"></div>
    ...
    <div v-for="p in displayProducts" :key="p.id" class="p-card" @click="addToCart(p)">
      <div class="p-info"><div class="p-title">{{ p.name }}</div><div class="p-qty">庫存: <span style="font-weight:900;" :style="{color: p.current_stock < 5 ? '#ef4444' : '#10b981'}">{{ p.current_stock }}</span> 件</div></div>
    ...
    <div v-if="cart.length > 0" class="float-bar" @click="showCheckoutModal = true">
      <div class="badge-num">{{ totalItems }}</div><div style="flex:1; margin-left:15px; font-weight:800; color:white;">件商品</div>
      <div class="float-total">$ {{ totalRevenue }}</div><div class="float-btn">結帳 ➔</div>
    </div>
...
.top-card { box-shadow: 0 10px 30px rgba(0,0,0,0.03); }
.form-item label { display: block; margin-bottom: 8px; font-weight: 800; font-size: 13px; color: #1e293b; }
.modern-inp, .modern-select { width: 100%; border: 1px solid #cbd5e1; padding: 12px; border-radius: 12px; font-weight: 700; color: #1e293b; outline: none; background: #f8fafc;}
