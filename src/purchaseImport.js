...
export function installPurchaseImport(store) {
  let currentRows = []
  let existingTxns = []
  let authUser = null
  let observer = null
  ...
  const openModal = () => {
      ...
      <div class="purchase-import-panel" role="dialog" aria-modal="true" aria-label="快速匯入購買紀錄">
        ...
        <div class="purchase-import-body">
          <div id="purchase-import-input-step">
            <div class="purchase-import-tip">需要欄位：客戶名稱／行動電話、銷售日期；系統亦會讀取「教練、名稱、店別／銷售門市」。</div>
    ...
    root.querySelector('#purchase-import-file').addEventListener('change', async e => {
      ...
      if (!file) return
      root.querySelector('#purchase-import-text').value = await file.text()
    })
  }
  ...
  async function analyzeInput() {
    ...
    const text = root.querySelector('#purchase-import-text')?.value || ''
    const rows = rowsFromGrid(parseGrid(text))
    if (!rows.length) {
      alert('讀不到可匯入資料。請連同 Excel 標題列一起複製；至少要有「銷售日期」及「客戶名稱／行動電話」。')
      return
    }
  ...
  function reclassifyRows() {
    ...
    for (const row of ordered) {
      ...
      const priorDbPurchase = existingTxns.some(txn => {
        if (!txnIdentityMatches(txn, row)) return false
        const date = String(txn.created_at || '').slice(0, 10)
        return date < row.date && txn.type === 'income' && ['運動套票', '運動', '試堂'].includes(txn.category)
      })
      row.isNewCustomer = !priorDbPurchase && !seenFilePurchase.has(identity)
      seenFilePurchase.add(identity)
  ...
  function packageOptionsHtml(selected = '') {
    const first = `<option value="">請選擇項目</option>`
    return first + Object.entries(PACKAGE_OPTIONS).map(([key, pkg]) =>
      `<option value="${key}" ${key === selected ? 'selected' : ''}>${esc(pkg.label)}</option>`
    ).join('')
  }
  ...
  function renderReview() {
    ...
    const unresolved = currentRows.filter(r => !r.packageKey).length
    ...
    review.innerHTML = `
      ...
      <div class="purchase-import-bulk">
        ...
        <span>原 Excel 金額不會匯入。</span>
      </div>
      <div class="purchase-import-list">
        ${currentRows.map(row => renderRow(row)).join('')}
      </div>
      <div class="purchase-import-footer">
        ...
        <div class="purchase-import-footer-right">
          ...
          <button type="button" class="purchase-import-primary" id="purchase-import-confirm" ${unresolved === currentRows.length ? 'disabled' : ''}>確認匯入已勾選紀錄</button>
        </div>
      </div>`
  ...
  function renderRow(row) {
    ...
    const clientBadge = row.matchedClient
      ? (row.isNewCustomer ? '<span class="purchase-import-badge new">舊客底・首次購買</span>' : '<span class="purchase-import-badge muted">已配對客戶</span>')
    ...
    return `
      <div class="purchase-import-row ${duplicateClass}">
        ...
        <div class="purchase-import-person">
          ...
          <div class="purchase-import-badges">${duplicateBadge}${clientBadge}${internBadge}</div>
  ...
  function updateSelectionCount() {
    ...
    const unresolved = currentRows.filter(r => r.selected && !r.packageKey).length
    target.textContent = `已選 ${selected} 筆${unresolved ? ` · ${unresolved} 筆未選項目` : ''}`
    const confirm = root.querySelector('#purchase-import-confirm')
    if (confirm) confirm.disabled = selected === 0 || unresolved > 0
  }
  ...
  async function commitImport() {
    ...
    try {
      ...
      for (const row of ordered) {
        ...
        if (!client) {
          const payload = {
            ...
            status: 'active',
            join_date: row.date,
            handled_by: row.coach || store.currentUser || '匯入',
        ...
        const txnPayload = {
          ...
          staff: row.coach || store.currentUser || '匯入',
          handled_by: row.coach || store.currentUser || '匯入',
      ...
      for (const [clientId, earliest] of earliestDates) {
        const client = clientById.get(clientId)
        const increment = packageIncrements.get(clientId) || 0
        const update = { status: 'active', join_date: earliest }
      ...
      if (store.isSyncing) await store.syncAll()
      await store.syncAll()
